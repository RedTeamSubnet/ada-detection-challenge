...
- Browser detection contributes 90% of the local score, weighted per browser.
- Headless detection contributes 10%, based on accuracy across scheduled driver runs.
- A wrong or missing headless verdict loses its proportional share instead of zeroing
  the complete score.
- Browser-detection collisions receive reduced credit (0.1 instead of 1.0 for
  that run).
- **Human gate:** any browser or headless detection during a human task makes the
  final score zero.
- Missing a run earns neither browser-detection nor headless-detection credit for that
  scheduled unit.
