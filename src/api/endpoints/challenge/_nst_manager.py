...
def run_nstbrowser(
    ...
    try:
        _container = docker_client.containers.run(
            ...
            environment={
                "TOKEN": config.challenge.nstbrowser.api_key.get_secret_value(),
                "PORT": "8848",
            },
            detach=True,
        )
        ...
        docker_client.api.connect_container_to_network(
            _container.name, network_names[1]
        )
...
def create_nst_profile() -> tuple[str, int] | None:
    ...
    try:
        ...
        payload = config.challenge.verification.extra
        ...
        if response.ok:
            ...
            port = profile_data.get("data").get("port")
            logger.success(
                f"Successfully created NSTBrowser profile with ID: {profile_id}, Port: {port}"
            )
...
def delete_nst_profile(profile_id: str) -> None:
    ...
    try:
        ...
        logger.success(
            f"Successfully deleted NSTBrowser profile with ID: {profile_id}."
        )
